...
results_base = 'bitchute_results'
check_dir(results_base)

popular_path = os.path.join(results_base, 'popular_videos')
...
trending_videos_path = os.path.join(results_base, 'trending_videos')
trending_tags_path = os.path.join(results_base, 'trending_tags')
...
all_path = os.path.join(results_base, 'all_videos')
check_dir(all_path)
rv, tags = b.get_recommended_videos(type='all')
t = get_time()
rv.to_csv(os.path.join(all_path, t+'.csv'), sep='\t', index=None)

recommended_channels_path = os.path.join(results_base, 'recommended_channels')
