...
class DataProcessor:
    ...
    def parse_channel(self, data: Dict[str, Any], rank: int = 0) -> Channel:
        ...
        try:
            ...
            channel.name = self._safe_get(data, 'channel_name', 
                                        self._safe_get(data, 'name', 
                                                     data.get('title', '')))
            ...
            # Date information
            channel.created_date = self._safe_get(data, 'date_created', 
                                                data.get('created_at', ''))
    ...
    def analyze_videos(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            if 'upload_date' in df.columns:
                try:
                    upload_dates = pd.to_datetime(df['upload_date'], errors='coerce').dropna()
                    if not upload_dates.empty:
                        analysis['upload_patterns'] = {
                            ...
                            'uploads_by_day': upload_dates.dt.day_name().value_counts().to_dict()
                        }
                except Exception as e:
                ...
                for sub_count in df['subscriber_count'].dropna():
                    try:
                        if 'K' in str(sub_count).upper():
                            numeric_subs.append(int(float(str(sub_count).upper().replace('K', '')) * 1000))
                        elif 'M' in str(sub_count).upper():
                            numeric_subs.append(int(float(str(sub_count).upper().replace('M', '')) * 1000000))
                        else:
